...
top_10_tissues <- head(sorted_tissues, 10)

# Step 4: Plot the result using a bar plot
barplot(top_10_tissues, 
        names.arg = names(top_10_tissues),  # Use the tissue names as labels
        main = "Top 10 Tissues by Ntrk2 Expression", 
        ylab = "Average Ntrk2 Expression", 
        col = "skyblue", las = 2)  # Rotate labels for clarity

selected_samples <- unique(mouse_cortex_data@meta.data$SampleID[mouse_cortex_data@meta.data$Tissue %in% c("Ctx1.5","Ctx3", "Ctx2", "Ctx1", "CA1", "Hypoth", "HC")])
...
samples_to_keep:
Threshold: The minimum expression level of the gene that we are interested in (0.8 in this case).
Percentage Threshold: The percentage of cells in a sample that must have an expression level above the threshold to keep the sample (50% in this case).
Cell Types of Interest: The cell types you are focusing on (Neurons, Astrocytes, and Oligos).
