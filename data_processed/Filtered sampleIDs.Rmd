...
top_12_tissues <- head(sorted_tissues, 12)
top_12_tissues <- as.data.frame(top_12_tissues)

row.names(top_12_tissues)
paste(rownames(top_15_tissues), collapse = ", ")



top_12_tissues_vector <- setNames(as.numeric(top_12_tissues[, 1]), rownames(top_12_tissues))
barplot(top_12_tissues_vector, 
...
selected_samples <- unique(mouse_cortex_data@meta.data$SampleID[mouse_cortex_data@meta.data$Tissue %in% c("Hypoth", "Ctx3", "Ctx1.5", "Ctx2", "MBv", "Amygd", "HC", "OB", "MBd", "Ctx1", "StriatDor", "CA1")])
...
#top 10 tissues resulted in 22 samples, top 12 gave 71 and top 15 gave ~ 83
```
